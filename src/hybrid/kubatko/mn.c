...
int
rpoisson(krand_t *kr, double lambda)
{
	int x = 0;
	double p, s;
	double u = kr_drand(kr);
...
/*
 * Approximation, sum(n) may not be equal to N!
 */
void
rmultinom2(krand_t *kr, int N, double *p, int (*n)[], size_t len)
{
	int i;
	int n_sum = 0;
	double n_ratio;
	...
	for (i=0; i<len; i++) {
		(*n)[i] = rpoisson(kr, N*p[i]);
		n_sum += (*n)[i];
	}

	n_ratio = (double)N / n_sum;
...
void
rmultinom(krand_t *kr, int N, double *p, int (*n)[], size_t len)
{
	double p_cum[len];
	double u;
	int i;
...
mcmc(FILE *fp, krand_t *kr, int y, gt3_t *abc, int sampling_period, int samples, double *y_max, double *t_max, double *ll_max)
{
	...
	int n[3] = {abc->nab, abc->nbc, abc->nac};
	...
	do {
		...
		//propose_pmultinom(kr, p0, &p1, 3);
		rmultinom2(kr, n_sum, p1, &n, 3);
		ll1 = llik(y1, t1, n[0], n[1], n[2]);
		ll1 += lmultinom(n, p1, 3);

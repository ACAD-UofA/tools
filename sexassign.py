...
def parse_args():
    ...
    parser.add_argument("-n", "--min-reads", type=int, default=1000, help="exclude samples with fewer reads than this [%(default)s]")
    parser.add_argument("-l", "--min-length", type=int, default=1000000, help="exclude contigs shorter than this [%(default)s]")
    parser.add_argument("-X", "--chrX", default="X,chrX,ChrX", help="list of X linked contigs [%(default)s]")
    parser.add_argument("-x", "--exclude-contigs", default="Y,chrY,ChrY,M,MT,Mt", help="list of contigs to exclude [%(default)s]")
    parser.add_argument("-i", "--include-contigs", default="", help="list of contigs to include [%(default)s]")
    parser.add_argument("-B", "--polar-bear", action="store_true", default=False, help="Use polar bear X linked scaffolds [%(default)s]")
    ...
    if args.polar_bear:
        # X-linked scaffolds as identified by mapping UrsMar1.0 scaffolds
        # onto the chromosome level CanFam3.1 assembly.
        args.chrX = ["KK498507.1","KK498524.1","KK498558.1","KK498591.1",
                    "KK498592.1","KK498613.1","KK498620.1","KK498621.1",
                    "KK498625.1","KK498626.1","KK498633.1","KK498654.1",
                    "KK498655.1","KK498666.1","KK498668.1","KK498669.1",
                    "KK498670.1","KK498681.1","KK498702.1","KK498740.1",
                    "KK498766.1","KK498779.1","KK498782.1","KK498829.1",
                    "KK498842.1","KK499341.1","KK499355.1","KK499613.1"]
    else:
        args.chrX = args.chrX.split(",")

    if args.exclude_contigs:
        args.exclude_contigs = set(args.exclude_contigs.split(","))
    else:
        args.exclude_contigs = set()

    if args.include_contigs:
